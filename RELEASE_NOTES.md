...
### Nowe
- Obsługa `PUT /testdata/certificates/{serialNumber}`, umożliwiająca aktualizację daty ważności certyfikatu na środowiskach testowych. `ksefClient.updateCertificate()` - przykład użycia `CertificateIntegrationTest.updateCertificate_ChangesValidTo()`
- `ksefClient.getResponseHeaderCaptureHandler().subscribe(...)` — subskrypcja pozwalająca odczytać nagłówki odpowiedzi (np. `X-System-Warning`) dla klienta SDK, bez zmiany sygnatur istniejących metod. Konfiguracja przez `ResponseHeaderObservationProperties.ResponseHeaderObservationOptions` (`enabled`, `headerNames`), domyślnie wyłączona. Odczyt nagłówków poprzez `ksefClient.getResponseHeaderCaptureHandler().getCaptured()`. Po każdym żądaniu należy wyczyścić zasubskrybowane nagłówki, przykład użycia w `ResponseHeaderObservationIntegrationTests`.
- Dodano obsługę endpointów identyfikatorów zbiorczych (`/collective-identifiers`):
    ...
    - `POST /collective-identifiers/invoices` — lista faktur wchodzących w skład wskazanych identyfikatorów zbiorczych (z paginacją).
      przykłady użycia w `CollectiveIdentifiersIntegrationTests`
...
- Dodano scenariusze testowe dla uprawnienia `INTROSPECTION` - przykłądy użycia w `IntrospectionPermissionIntegrationTests`
- Dodano scenariusze testowe dla tokenów KSeF - przykłądy użycia w `KsefTokenIntegrationTest` i `RevokedPermissionsKsefTokenLifecycleIntegrationTests`

### Zmodyfikowane
- `KsefTokenPermissionType`: dodano brakującą wartość `Introspection` i `CollectiveIdentifierManage` oraz usunięto `PeppolId`, dostosowując enum do aktualnego kontraktu API.
- Wprowadzono nowy sposób zapisu dla rejestru zmian (`RELEASE_NOTES.md`), poprzednie wpisy merytorycznie są bez zmian, jedynie została odwrócona kolejność wpisów.
...
- `OpenBatchSessionRequestBuilder.withBatchFile(fileSize, fileHash)`: pole `compressionType` jest opcjonalny i domyślnie przyjmuje wartość `TarGz`.
...
- `InvoiceExportRequest.compressionType`: zmieniono na domyślną wartość `TarGz`.
- Dodano brakujący enum `VatPefSp` w `InvoiceMetadataInvoiceType` i zmieniono nazwę dla `KorVatRr`
